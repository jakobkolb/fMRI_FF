...
for f in range(tp.number_of_runs):
    ...
    print 'average times that high reward is left is', (inversions[:,1]==1).sum()/float(number_of_trials)
    print 'average times that marker_1 is left on options slide is', (inversions[:,2]==1).sum()/float(number_of_trials)
    ...
    print 'the total duration of the run is', str(datetime.timedelta(seconds=int(round(sum(sum(timing[:,:])),0)*tp.fmri_time)))
    print 'the total duration in TRs is', sum(sum(timing))
    ...
    with open(output_folder + 'run_'+`f+1`+'_parameters_.p', 'wb') as fb:
